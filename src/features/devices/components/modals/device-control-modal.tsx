import type { TDeviceConfig } from '../types';
import type { TDevice, TDeviceFeature } from '@/lib/api/devices/device.service';

import * as React from 'react';

import { Text, TouchableOpacity, View } from '@/components/ui';
import { Modal } from '@/components/ui/modal';
import { translate } from '@/lib/i18n';
import { isPrimaryFeature } from '@/lib/utils/device-feature-helper';

type TDeviceControlModalProps = {
  modalRef: React.RefObject<any>;
  device: TDevice;
  activeFeature?: TDeviceFeature;
  config: TDeviceConfig;
};

export function DeviceControlModal({
  modalRef,
  device,
  activeFeature,
  config,
}: TDeviceControlModalProps) {
  const primaryFeatures = device.features?.filter(isPrimaryFeature) ?? [];

  return (
    <Modal ref={modalRef} snapPoints={config.modalSnapPoints} title={device.name}>
      <View className="mt-4 flex-1 px-1">
        <Text className="mb-1 text-xl font-bold text-neutral-900 dark:text-white">
          {device.name}
        </Text>
        {activeFeature && (
          <Text className="mb-4 text-sm font-medium text-neutral-500">
            {translate('base.module')}: {activeFeature.name || activeFeature.code}
          </Text>
        )}
        {primaryFeatures.length > 0
          ? (
              <View className="flex-row flex-wrap gap-3">
                {primaryFeatures.map((f) => {
                  const featureOn = f.currentValue === 1;
                  return (
                    <TouchableOpacity
                      key={f.id}
                      className={`h-24 min-w-[45%] flex-1 justify-between rounded-xl p-3 ${featureOn ? 'bg-[#A3EC3E]' : 'bg-neutral-100 dark:bg-neutral-800'}`}
                      onPress={() => {
                        // TODO: Toggle feature via socket
                        console.log('Toggle:', f.code);
                      }}
                    >
                      <View className={`size-3 rounded-full ${featureOn ? 'bg-white' : 'bg-neutral-300 dark:bg-neutral-600'}`} />
                      <Text className={`text-lg font-semibold ${featureOn ? 'text-black' : 'text-neutral-800 dark:text-white'}`}>
                        {f.name || f.code}
                      </Text>
                    </TouchableOpacity>
                  );
                })}
              </View>
            )
          : (
              <Text className="mt-10 text-center text-neutral-500">
                {translate('base.noFeatureControl')}
              </Text>
            )}
      </View>
    </Modal>
  );
}
