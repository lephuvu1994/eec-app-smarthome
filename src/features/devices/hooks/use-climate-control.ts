...
export function useClimateControl(device: TDevice, entity: TDeviceEntity) {
  ...
  useDeviceEvent(device?.id || '', useCallback((data: { entityCode?: string; state?: any; value?: any, attributes?: Record<string, any> }) => {
    if (data.entityCode === entity?.code) {
      ...
      if (data.attributes) {
        if (data.attributes.temperature !== undefined) setTargetTemp(Number(data.attributes.temperature));
        if (data.attributes.current_temperature !== undefined) setCurrentTemp(Number(data.attributes.current_temperature));
        if (data.attributes.mode !== undefined) setMode(String(data.attributes.mode));
      }
    }
  }, [entity?.code]));
  ...
  const handleSetTemperature = async (newTemp: number) => {
    if (newTemp < 16 || newTemp > 30) return; // Standard AC limits
