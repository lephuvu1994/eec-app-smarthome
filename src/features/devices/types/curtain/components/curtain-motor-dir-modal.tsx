...
type Props = {
  ...
  onSelectDir: (dir: 'forward' | 'reverse') => void;
};
...
export function CurtainMotorDirModal({
  ...
  const handleSelect = (dir: 'forward' | 'reverse') => {
    onSelectDir(dir);
    ...
    <Modal ref={modalRef} snapPoints={[insets.bottom + HEIGHT * 0.35]} title={translate('deviceDetail.shutter.advanced.motorDir')}>
      <View className="p-5 pb-10">
        <View className="rounded-2xl bg-white p-2 shadow-sm dark:bg-neutral-800">
          ...
          <View className="mx-4 h-[1px] bg-neutral-100 dark:bg-neutral-700" />

          <TouchableOpacity
            className="flex-row items-center justify-between rounded-xl px-4 py-4"
            disabled={isControlling}
            onPress={() => handleSelect('reverse')}
            activeOpacity={0.7}
          >
            <View className="flex-row items-center gap-4">
