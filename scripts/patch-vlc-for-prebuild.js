...
function patchFile(filePath) {
  ...
  if (!content.includes('applyNativeModulesAppBuildGradle')) return false;
  if (content.includes('autolinkLibrariesWithApp()')) return false; // already patched
  if (!content.includes(OLD_LINE)) return false;
...
try {
  const resolvePath = require.resolve('react-native-vlc-media-player/expo/android/withGradleTasks.js', {
    paths: [process.cwd()],
  });
...
if (patched) {
  console.log('patch-vlc-for-prebuild: applied (anchor → autolinkLibrariesWithApp).');
} else {
  console.log('patch-vlc-for-prebuild: already patched or package not found, skip.');
}
process.exit(0);
